...
//	protected static final String DEFAULT_ENCRYPTION_USER = "cee-server";
	...
	public void init(String user, String password, String url) throws MalformedURLException {
		mUserName = user;
		mPassword = password;
		mURL = url;
		...
		mUserIdentifier = new UserIdentifier(mUserName, mDomainName, mProjectName);
	...
	private String getConfigString(int type) {
		...
		String extendedConfigString = "<deployment xmlns=\"http://xml.apache.org/axis/wsdd/\" "
				...
				+ "</handler>" + "</requestFlow>" + "</globalConfiguration >" + "</deployment>";
	 ...
	 */
	private void computeProjectAndDomain(String urlString) throws MalformedURLException {
	...
	public EngineConfiguration getEngineConfiguration() {
		...
		EngineConfiguration config = new FileProvider(new ByteArrayInputStream(configString.getBytes()));
	...
	public URL constructServiceURL(String service) {
		URL url;
		try {
			url = new URL(getURL() + "/axis/services/" + getSecurityToken() + service);
