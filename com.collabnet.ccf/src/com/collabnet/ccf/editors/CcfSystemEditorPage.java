...
public class CcfSystemEditorPage extends CcfEditorPage {
	...
	private void createControls(Composite composite) {		
		...
		case SW:
			...
			break;	
		case FS:
			headerLabel.setText("TFS Properties");
			initializeTfsValues();
			break;
		default:
			break;
	...
	public void doSave(IProgressMonitor monitor) {
		...
		try {
			...
			case FS:
				...
				if (getLandscape().getConfigurationFolder1() != null) {
					folder = new File(getLandscape().getConfigurationFolder1());
					propertiesFile1 = new File(folder, "tfs.properties");
				}			
				if (getLandscape().getConfigurationFolder2() != null) {
					folder = new File(getLandscape().getConfigurationFolder2());
					propertiesFile2 = new File(folder, "tfs.properties");
				}
				initializeTfsValues();
				break;
			default:
	...
	private void initializeTfsValues() {
		url = properties.getProperty(Activator.PROPERTIES_TFS_URL, "");
		user = properties.getProperty(Activator.PROPERTIES_TFS_USER, "");
		password = Activator.decodePassword(properties.getProperty(Activator.PROPERTIES_TFS_PASSWORD, ""));
		attachmentSize = properties.getProperty(Activator.PROPERTIES_TFS_ATTACHMENT_SIZE, "10485760");
	}
	...
	private void setTfsProperties() throws Exception {
		String password = properties.getProperty(Activator.PROPERTIES_TFS_PASSWORD);
		boolean passwordPreviouslyEncoded = password != null && password.startsWith(Activator.OBFUSCATED_PASSWORD_PREFIX);
		password = properties.getProperty(Activator.PROPERTIES_TFS_RESYNC_PASSWORD);
		
		properties.setProperty(Activator.PROPERTIES_TFS_URL, urlText.getText().trim());
		properties.setProperty(Activator.PROPERTIES_TFS_USER, userText.getText().trim());
		properties.setProperty(Activator.PROPERTIES_TFS_PASSWORD, Activator.encodePassword(passwordText.getText().trim(), passwordPreviouslyEncoded));		
		properties.setProperty(Activator.PROPERTIES_TFS_ATTACHMENT_SIZE, attachmentSizeText.getText().trim());
	}
